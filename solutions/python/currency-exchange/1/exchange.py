...
def get_number_of_bills(amount, denomination):
    ...
    money = float(amount / denomination)
    return int(money)
...
def exchangeable_value(budget, exchange_rate, spread, denomination):
    ...
    # Calculate the adjusted exchange rate (adding the spread fee)
    adjusted_exchange_rate = exchange_rate * (1 + spread_decimal)
    
    # Calculate how much foreign currency you get
    foreign_currency = budget / adjusted_exchange_rate
    
    # Calculate how many complete bills you can get
    number_of_bills = int(foreign_currency // denomination)
    
    # Calculate the total value in bills
